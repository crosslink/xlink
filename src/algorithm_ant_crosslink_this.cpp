...
void algorithm_ant_crosslink_this::add_link(links* lx, ANT_link_term *term, char **term_list, long offset)
{
	switch (translate_anchor_for_linking_) {
		case 2:
			algorithm_ant_link_this::add_link(lx, term, term_list, offset);
			break;
		...
		case 1:
			...
			//		std::cerr << "Something funny happened." << std::endl;
				if (string_compare(crossterm->term, result.c_str(), TRUE) == 0)
					algorithm_ant_link_this::add_link(lx, crossterm, term_list, offset);
			}
			break;
	}
}
